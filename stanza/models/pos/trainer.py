...
class Trainer(BaseTrainer):
    ...
    def predict(self, batch, unsort=True):
        ...
        # print('pos preds', preds)
        # print('self.vocab upos', self.vocab['upos'])
        # print('preds[0].tolist()', preds[0].tolist())
        ...
        # print('document sent', document.sentences)
        # print('document sents', document.sentences)
        # print('document text', type(document.text))
        # print('document DICT', document.to_dict())

        pred_tokens = [[[upos_seqs[i][j], xpos_seqs[i][j], feats_seqs[i][j]] for j in range(sentlens[i])] for i in range(batch_size)]
        if unsort:
            pred_tokens = utils.unsort(pred_tokens, orig_idx)
            # print('pred_tokens in trainer', pred_tokens)
        return pred_tokens
